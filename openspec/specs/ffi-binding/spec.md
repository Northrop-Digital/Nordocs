# ffi-binding Specification

## Purpose
The C-ABI / .NET binding (`nordocs-ffi`) over the `nordocs-core` façade: panic-guarded exports for multi-format compile, markdown, preview, authoring, validation, and an opaque source-map session handle, with safe error marshalling, explicit buffer/string lifetimes, and a generated C# surface.
## Requirements
