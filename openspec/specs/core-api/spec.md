...
## Purpose
The binding-agnostic service façade exposed by `nordocs-core`: every document operation (compile/render, markdown, preview, authoring, validation, catalogue introspection) as a structured-result function with no terminal output, process exit, or external-viewer side effects, so the CLI, the FFI, and a future WASM shell call the same core logic.
## Requirements
