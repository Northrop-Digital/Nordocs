...
impl NordocsWorld {
    ...
    pub fn new(main_source: impl Into<String>) -> Self {
        Self::with_inputs(main_source, &[])
    }

    /// Build a world from a composed `.typ` source string with `sys.inputs`.
    ///
    /// `inputs` is an ordered list of `(key, value)` pairs injected into the
    /// standard library so the document sees them as `sys.inputs.<key>` string
    /// values, mirroring the reference `TypstCompiler.SetSysInputs`. Passing an
    /// empty slice is byte-for-byte equivalent to [`NordocsWorld::new`] because
    /// `Library::default()` is itself `builder().build()` with an empty input
    /// dictionary.
    pub fn with_inputs(main_source: impl Into<String>, inputs: &[(String, String)]) -> Self {
        ...
        let dict: Dict = inputs
            .iter()
            .map(|(key, value)| (Str::from(key.clone()), Value::Str(Str::from(value.clone()))))
            .collect();
...
/// Extend the world with IDE capabilities (`typst-ide`).
///
/// The only required method is [`IdeWorld::upcast`], a shim that exists because
/// trait upcasting (`&dyn IdeWorld` → `&dyn World`) is still unstable in Rust.
/// `NordocsWorld` already implements [`World`], so `upcast` simply returns
/// `self`. The optional `packages`/`files` hooks are left at their defaults:
/// they only enhance autocompletion, which is irrelevant to the click-to-source
/// and cursor-to-preview jumps this trait unlocks (see
/// [`crate::compiler::CompiledDoc::jump_from_click`]).
impl typst_ide::IdeWorld for NordocsWorld {
...
mod tests {
    ...
    #[test]
    fn world_with_inputs_exposes_sys_inputs() {
        // The document echoes an injected sys.inputs value; compiling it and
        // confirming a successful PagedDocument proves the value resolved.
        let world =
            NordocsWorld::with_inputs("#sys.inputs.foo", &[("foo".to_string(), "bar".to_string())]);
        let compiled = typst::compile::<typst::layout::PagedDocument>(&world);
        assert!(
            compiled.output.is_ok(),
            "reading an injected sys.inputs.foo should compile cleanly"
        );
    }

    #[test]
    fn world_with_inputs_missing_key_is_unset() {
        // Without the input, sys.inputs.foo is absent and the dictionary access
        // must fail to compile, proving inputs are actually scoped.
        let world = NordocsWorld::with_inputs("#sys.inputs.foo", &[]);
        let compiled = typst::compile::<typst::layout::PagedDocument>(&world);
        assert!(
            compiled.output.is_err(),
            "absent sys.inputs.foo should not resolve"
        );
    }
}
