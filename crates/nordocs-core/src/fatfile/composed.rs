...
/// Compile a composed fat file into a retained [`CompiledDoc`].
///
/// Identical to [`render_to_pdf`] but retains the laid-out document so a binding
/// can export it to PDF, SVG, or PNG without recompiling. The manifest images
/// are materialised into the compiler's virtual filesystem before compilation.
pub fn render_to_session(src: &str) -> Result<crate::compiler::CompiledDoc> {
    let images = resolve_images(src)?;
    crate::compiler::compile_with_images(src, &images)
}
