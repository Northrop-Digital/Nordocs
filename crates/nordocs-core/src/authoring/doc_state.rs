...
///
/// Both variants carry the blake3 `hash` of the image bytes so callers (the CLI
/// `--json` envelope, the FFI) can report it without recomputing the digest.
...
pub fn embed_image(path: &Path, image_name: &str, image_bytes: &[u8]) -> Result<ImageEmbed> {
    ...
    if let Some(existing) = doc.images.iter().find(|img| img.name == image_name) {
        if existing.hash == hash {
            return Ok(ImageEmbed::AlreadyPresent { hash });
        }
...
mod tests {
    ...
    fn embed_image_is_idempotent_on_identical_content() {
        ...
        let same_hash = blake3::hash(b"SAME").to_hex().to_string();
        assert_eq!(
        ...
        assert_eq!(
            ...
            ImageEmbed::AlreadyPresent { hash: same_hash }
        );
